...
export default function CatalogoPublico() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: cfg } = await db
        .from('configuracion')
        .select('clave, valor')
        .in('clave', ['empresa_telefono', 'empresa_nombre']);
      if (cfg) {
        cfg.forEach((c: any) => {
          if (c.clave === 'empresa_telefono') setTelefono(c.valor?.replace(/\D/g, '') || '');
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header */}
      <div className="bg-[#185FA5] text-white py-8 px-4 text-center shadow-md">
        <div className="max-w-4xl mx-auto">
          <img
            src="/icons/icon-128.png"
            alt="logo"
            className="w-16 h-16 object-contain rounded-xl mx-auto mb-3 bg-white/10 p-1"
            onError={e => (e.currentTarget.style.display = 'none')}
          />
          <h1 className="text-2xl font-bold">{nombre}</h1>
          <p className="text-blue-100 text-sm mt-1">Tapicería &amp; Ebanistería · Catálogo de productos</p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 space-y-5">
        ...
        {!loading && filtered.length === 0 && (
          <div className="text-center py-20 text-gray-400">
            ...
            <p className="text-sm mt-1">Vuelve pronto o contáctanos por WhatsApp</p>
            {telefono && (
              <a
        ...
        {!loading && (
          <p className="text-center text-xs text-gray-400 pt-4 pb-8">
            {nombre} · Todos los precios en RD$
          </p>
        )}
      </div>

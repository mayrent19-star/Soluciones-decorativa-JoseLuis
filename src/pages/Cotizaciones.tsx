...
const estadosBadge: Record<string, 'default' | 'secondary' | 'destructive' | 'outline'> = {
  'Borrador': 'outline', 'Enviada': 'default', 'Aprobada': 'secondary', 'Rechazada': 'destructive', 'Vencida': 'destructive'
};
...
export default function Cotizaciones() {
  ...
  const [form, setForm] = useState<any>({ id_cliente: '', estado: 'Borrador', fecha: new Date().toISOString().slice(0, 10), notas: '' });
  const [cotItems, setCotItems] = useState<any[]>([{ descripcion: '', cantidad: 1, precio_unitario: 0 }]);

  const load = async () => {
    const [cots, cls] = await Promise.all([fetchAll('cotizaciones'), fetchAll('clientes', 'nombre_completo', true)]);
    setItems(cots);
    setClientes(cls);
  };
  useEffect(() => { load(); }, []);
  ...
  const openNew = () => {
    setForm({ id_cliente: '', estado: 'Borrador', fecha: new Date().toISOString().slice(0, 10), notas: '' });
  ...
  const generatePDF = async (cot: any) => {
    ...
    <style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Segoe UI',sans-serif;padding:40px;max-width:800px;margin:auto;color:#1a1a1a}.header{display:flex;justify-content:space-between;border-bottom:3px solid #0C717F;padding-bottom:20px;margin-bottom:30px}.brand{display:flex;align-items:center;gap:14px}.brand img{width:70px;height:70px;object-fit:contain;border-radius:8px}.brand-info h1{color:#0C717F;font-size:16px}.brand-info p{font-size:11px;color:#666}.inv-info{text-align:right}.inv-info h2{color:#0C717F;font-size:24px}.inv-info p{font-size:12px;color:#666}.section{margin-bottom:20px}.section-title{font-size:12px;font-weight:700;color:#0C717F;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;border-bottom:1px solid #eee;padding-bottom:4px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:6px 20px}.field label{font-size:10px;color:#888}.field p{font-size:13px;font-weight:500}table.items{width:100%;border-collapse:collapse;margin:10px 0}table.items th{background:#0C717F;color:white;padding:8px 12px;font-size:11px;text-align:left}table.items td{padding:8px 12px;font-size:12px;border-bottom:1px solid #eee}.text-right{text-align:right}.totals{display:flex;justify-content:flex-end;margin-top:12px}.totals-table tr td{padding:4px 12px;font-size:12px}.totals-table .total td{font-weight:700;font-size:15px;border-top:2px solid #0C717F;color:#0C717F}.warranty{margin-top:30px;padding:12px;background:#f8f8f8;border-radius:6px;font-size:11px;color:#666;border-left:3px solid #0C717F}.footer{margin-top:30px;text-align:center;font-size:10px;color:#999;border-top:1px solid #eee;padding-top:12px}</style></head><body>
    ...
    <div class="totals"><table class="totals-table"><tr><td>Subtotal</td><td class="text-right">${formatCurrency(cot.subtotal)}</td></tr><tr><td>ITBIS (18%)</td><td class="text-right">${formatCurrency(cot.itbis)}</td></tr><tr class="total"><td>Total</td><td class="text-right">${formatCurrency(cot.total)}</td></tr></table></div>
    ${cot.notas ? `<div class="section" style="margin-top:20px"><div class="section-title">Notas</div><p style="font-size:12px;color:#555">${cot.notas}</p></div>` : ''}
      ...
      <div className="border rounded-lg overflow-x-auto bg-card">
        <Table>
          ...
          <TableBody>
            ...
            {filtered.map(c => (
              <TableRow key={c.id}>
                <TableCell className="font-medium">{c.numero_cotizacion}</TableCell>
                <TableCell className="text-muted-foreground">{clientes.find((cl: any) => cl.id === c.id_cliente)?.nombre_completo || '—'}</TableCell>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div className="grid gap-1.5">
                ...
                <Select value={form.estado} onValueChange={v => setForm({ ...form, estado: v })}>
                  ...
                  <SelectContent>{['Borrador', 'Enviada', 'Aprobada', 'Rechazada', 'Vencida'].map(e => <SelectItem key={e} value={e}>{e}</SelectItem>)}</SelectContent>
                </Select>
              </div>
            </div>
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div className="grid gap-1.5"><Label className="text-xs">Válida hasta</Label><Input type="date" value={form.fecha_vencimiento || ''} onChange={e => setForm({ ...form, fecha_vencimiento: e.target.value })} /></div>
            </div>
                ...
                {cotItems.map((item, idx) => (
                  <div key={idx} className="grid grid-cols-12 gap-2 items-end">
                    ...
                    <div className="col-span-2 text-right">
                      <span className="text-sm font-medium">{formatCurrency(item.cantidad * item.precio_unitario)}</span>
          ...
          {detailDialog && (
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div><span className="text-muted-foreground text-xs">Estado</span><p><Badge variant={estadosBadge[detailDialog.estado]}>{detailDialog.estado}</Badge></p></div>
                <div><span className="text-muted-foreground text-xs">RNC Empresa</span><p>{detailDialog.rnc_empresa || '—'}</p></div>
                <div><span className="text-muted-foreground text-xs">RNC Cliente</span><p>{detailDialog.rnc_cliente || '—'}</p></div>
              </div>
              <Table>
                <TableHeader><TableRow><TableHead>Descripción</TableHead><TableHead className="text-right">Cant.</TableHead><TableHead className="text-right">P.Unit</TableHead><TableHead className="text-right">Total</TableHead></TableRow></TableHeader>
                <TableBody>
                  {detailDialog.items?.map((i: any) => (
                    <TableRow key={i.id}><TableCell>{i.descripcion}</TableCell><TableCell className="text-right">{i.cantidad}</TableCell><TableCell className="text-right">{formatCurrency(i.precio_unitario)}</TableCell><TableCell className="text-right">{formatCurrency(i.total || i.cantidad * i.precio_unitario)}</TableCell></TableRow>
                  ))}
                </TableBody>
              </Table>
              <div className="text-right font-bold text-primary">Total: {formatCurrency(detailDialog.total)}</div>
            </div>
          )}
        </DialogContent>
      </Dialog>
